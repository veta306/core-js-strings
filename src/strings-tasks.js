...
function sumOfCodes(str) {
  let sum = 0;
  if (str) for (let i = 0; i < str.length; i += 1) sum += str.charCodeAt(i);
  return sum;
}
...
function isPalindrome(str) {
  return (
    str.toLowerCase().match(/[a-z]/g).join('') ===
    reverseString(str).toLowerCase().match(/[a-z]/g).join('')
  );
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (letter) => cipher[cipher.indexOf(letter) + 13]
  );
}
